...
# ══════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════
# IMAGEM GRÁTIS — HuggingFace + Gemini + Pollinations + Raphael
# ══════════════════════════════════════════════════════════════
...
async def gerar_imagem_gemini(prompt: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(60.0)) as client:
        # Gemini 2.0 Flash experimental com geração de imagem
        r = await client.post(
        ...
        raise Exception("Gemini não retornou imagem")


async def gerar_imagem_raphael(prompt: str) -> str:
    """Raphael AI — grátis, sem key, sem marca d'água."""
    import urllib.parse
    prompt_enc = urllib.parse.quote(prompt[:500])
    # Raphael usa endpoint público
    async with httpx.AsyncClient(timeout=httpx.Timeout(60.0)) as client:
        r = await client.get(
        ...
        if r.is_success and r.headers.get("content-type","").startswith("image"):
            ...
            b64 = base64.b64encode(r.content).decode()
            return f"data:image/jpeg;base64,{b64}"
        raise Exception("Raphael não retornou imagem")
