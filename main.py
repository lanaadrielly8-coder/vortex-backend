...
CLOUDO_MODELO = """You are VORTEX DIRECTOR — the most aggressive and precise screenplay AI in the world.

You don't write generic scripts. You SURGICALLY craft content. Every word has purpose. Every second has function. Every cut has intention.

You think like:
• Christopher Nolan structuring narrative
• MrBeast calculating retention
• A psychologist manipulating emotion
• A copywriter converting attention into action
• A TikTok algorithm engineer maximizing watch time

GOLDEN RULE: If the viewer can predict what comes next, the script is WRONG. Rewrite it.

MINIMUM QUALITY STANDARD:
• Hook: must stop the scroll in 0.3 seconds
• Every scene must generate at least 1 new question
• The twist must be IMPOSSIBLE to predict
• CTA never asks for likes — it creates obsession
• Minimum acceptable score: 8/10 in all dimensions

NEVER:
• Start with a question in the hook — disturbing statements convert 3x more
• Make slow introductions — the first 3 seconds are EVERYTHING
• Use robotic, corporate or overly didactic language
• Deliver a script with score < 40 — rewrite first
• Create generic scenes without visual and audio specification
• Ask for likes, subscription or shares directly
• Use "3 tips" or "5 steps" format — dead format
• Make predictable conclusions — the last second must be the strongest

ALWAYS:
• Treat every second as if it cost $1,000 in paid media
• Think like algorithm: what will hold? what will make them comment?
• Create at least ONE scene nobody in the niche has done before
• Use real data, real cases, specific numbers — specificity converts
• End with something that forces the viewer to open the profile or comment
• Adapt vocabulary, references and rhythm to the niche and creator's DNA
• When in doubt between two approaches — choose the most disturbing one

FORMATS THAT WORK IN 2026:
• Immersive POV — viewer IS the character
• Progressive revelation — information dropped in doses
• Initial contradiction — affirm the opposite of common sense
• Specific numbers — "23 minutes" converts more than "a little time"
...
def montar_system_vortex(usar_cloudo: bool = False, extra: str = "", canal_id: str = "default") -> str:
    ...
    if usar_cloudo:
        ...
        base = CLOUDO_MODELO  # Vortex é SEMPRE o diretor viral
        if contexto:
            base += f"\n🎯 CRIADOR: {contexto}"
            base += "\nUse SEMPRE esse perfil. Nicho, tom e plataformas guiam tudo."
    if extra: base += f"\n{extra}"
...
MÉDIA: X/10 — [VIRAL/POTENCIAL/RETRABALHAR]"""
    ...
    for tentativa in range(2):
        try:
            ...
            print(f"[VORTEX] ❌ Tentativa {tentativa+1} falhou: {e_retry}")
            if tentativa == 0:
                import asyncio as _asyncio
                await _asyncio.sleep(2)
                config["provedor"] = "gemini"  # força Gemini no retry
            else:
...
MÉDIA: X/10 — [VIRAL/POTENCIAL/RETRABALHAR]"""
    ...
    try:
        ...
        roteiro = msg.content[0].text
        return {"ok": True, "roteiro": roteiro}
    except Exception as e:
        raise HTTPException(500, f"Claude erro: {str(e)[:200]}")
...
10. Preview do próximo episódio"""
        ...
        prompt = f"Tema: {data.tema} | Formato: {data.formato} ({plataforma})"
        if contexto: prompt += f" | Criador: {contexto}"
...
Use os fatos reais acima. Seja específico — nomes reais, datas reais, locais reais."""
    ...
    roteiro, modelo = await gerar_texto(
        [{"role":"user","content":prompt}], system=system,
        max_tokens=MAX_TOKENS, provedor_preferido=provedor,
    )
...
{f"DADOS REAIS DA INTERNET (use esses!):\n{fatos_reais}\n" if fatos_reais else f"TENDÊNCIAS BASE:\n{', '.join(trends_base)}\n"}
...
async def gerar_imagem_wavespeed(prompt: str, endpoint: str = "wavespeed-ai/flux-dev") -> str:
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(30.0)) as client:
        r = await client.post(
            ...
            json={"prompt": prompt, "size": "1024*1024", "num_images": 1, "seed": -1, "output_format": "jpeg"},
        )
